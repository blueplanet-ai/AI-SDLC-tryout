...
| D24 | Live log details (added in step 5, 2026-09-24, open for owner review) | The participant ID is entered on the Live log screen ("Start session" in Setup leads there). The D17 "Export backup now" offer, and keeping the half-typed note after the tab closes (section 4), are built in step 5 together with End session. While a session runs, Setup and Studies show "Continue in Live log", and no other study can start a session (D12). "Find or add a screen" (Alt+S) picks an existing screen or adds the typed name to the list (FR1, D2). The recent feed shows the last 5 findings; there only the note can be fixed — screen and type are edited in Review (step 6). End session has no confirmation step. |
