...
| D27 | Feedback received (added in step 8, decided by the product owner 2026-09-25) | **Decided by the owner:** logged feedback cannot be edited; delete it (with confirmation, as D16) and add it again. Date received defaults to today; future dates are rejected. With 0 or 1 responses the status reads "&lt;n&gt; of 2 — below target"; with 2 or more, "&lt;n&gt; of 2 — target met". Each study row on the Studies list shows "Feedback: &lt;n&gt; of 2 — &lt;status&gt;". The feedback box shows the reminder "Paste the content only — no names, email addresses or signatures." The target stays fixed at 2 (not editable in the app). **Details chosen by Claude in step 8 (open for owner review):** the panel sits on the Summary screen below the summary text, in its own file `views/feedback.js`. "Future" means later than today on the laptop's clock and time zone; there is no earliest allowed date. The error reads "The date received cannot be in the future." Logged feedback is listed newest date received first (same date: last logged first), each with "Received &lt;YYYY-MM-DD&gt;" and a Delete button. In the feedback box, Enter adds a new line (pasted replies often have several lines); the "Add feedback" button adds it. The Studies list has a new "Feedback" column holding the owner's wording. The status is amber when below target and green when met (the words carry the meaning too). Feedback can be logged at any time, also while a session is running. |
